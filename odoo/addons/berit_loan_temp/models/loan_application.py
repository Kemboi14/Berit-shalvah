...
class BeritLoanApplication(models.Model):
    ...
    has_prior_loans = fields.Boolean(
        string='Has Prior Loans',
        compute='_compute_has_prior_loans',
        store=True
    )
    ...
    def _compute_maturity_date(self):
        ...
        for loan in self:
            if loan.disbursement_date and loan.loan_duration:
                maturity_date = loan.disbursement_date + timedelta(days=loan.loan_duration * 30)
    ...
    def _check_loan_duration(self):
        ...
        for loan in self:
            if not loan.has_prior_loans and loan.loan_duration > 3:
                raise ValidationError(_("First-time applicants can only apply for loans up to 3 months."))
            if loan.loan_duration < 1 or loan.loan_duration > 12:
                raise ValidationError(_("Loan duration must be between 1 and 12 months."))
    ...
    def _check_collateral_value(self):
        ...
        for loan in self:
            if loan.loan_amount > 0:
                total_collateral_value = sum(coll.estimated_value for coll in loan.collateral_ids if coll.is_verified)
                required_value = loan.loan_amount * 1.5
                
                if total_collateral_value < required_value:
                    raise ValidationError(_("Total verified collateral value must be at least 1.5× the loan amount (KES %s).") % required_value)
                
                # Policy: Collateral value should NOT exceed 1.5× loan amount
                if total_collateral_value > required_value:
                    raise ValidationError(_("Collateral value must not exceed 1.5× the loan amount (KES %s) per company policy.") % required_value)
    
    @api.constrains('crb_cleared', 'state')
    def _check_crb_clearance(self):
        """Validate CRB clearance before approval"""
        for loan in self:
            if loan.state in ['approved', 'disbursed', 'active'] and not loan.crb_cleared:
                raise ValidationError(_("CRB clearance must be verified before loan approval."))
    
    @api.model
    def create(self, vals):
        """Create loan application with sequence number"""
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('berit.loan.application') or _('New')
        return super(BeritLoanApplication, self).create(vals)
    ...
    def _send_submission_notification(self):
        ...
        template = self.env.ref('berit_loan.email_template_loan_submitted', raise_if_not_found=False)
        if template:
            template.send_mail(self.id)
    
    def _send_approval_notification(self):
        """Send approval notification email"""
        template = self.env.ref('berit_loan.email_template_loan_approved', raise_if_not_found=False)
        if template:
            template.send_mail(self.id)
    
    def _send_rejection_notification(self):
        """Send rejection notification email"""
        template = self.env.ref('berit_loan.email_template_loan_rejected', raise_if_not_found=False)
        if template:
            template.send_mail(self.id)
