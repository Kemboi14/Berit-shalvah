...
class BeritCollateral(models.Model):
    ...
    valuation_document = fields.Binary(
        string='Valuation Document',
        attachment=True
    )
    
    valuation_document_name = fields.Char(
        string='Valuation Document Name'
    )
    
    ownership_proof = fields.Binary(
        string='Ownership Proof',
        attachment=True
    )
    
    ownership_proof_name = fields.Char(
        string='Ownership Proof Name'
    )
    ...
    def create(self, vals):
        """Create collateral with automatic naming"""
        if not vals.get('valuation_document_name') and vals.get('valuation_document'):
        ...
        return super(BeritCollateral, self).create(vals)
