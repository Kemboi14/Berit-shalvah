...
class BeritRepaymentSchedule(models.Model):
    ...
    principal_amount = fields.Float(
        string='Principal Amount (KES)',
        required=True
    )
    
    interest_amount = fields.Float(
        string='Interest Amount (KES)',
        required=True
    )
    ...
    def _compute_penalty_amount(self):
        ...
        for schedule in self:
            if schedule.days_overdue > 0:
                ...
                schedule.penalty_amount = schedule.total_due * 0.01 * schedule.days_overdue
            else:
    ...
    def action_record_payment(self, amount_paid, payment_method=None, payment_reference=None):
    ...
    def send_reminder_email(self):
        ...
        template = self.env.ref('berit_loan.email_template_payment_reminder', raise_if_not_found=False)
        if template:
