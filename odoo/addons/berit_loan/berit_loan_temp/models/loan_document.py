...
class BeritLoanDocument(models.Model):
    ...
    def _compute_mime_type(self):
        ...
        for doc in self:
            if doc.filename:
                extension = doc.filename.split('.')[-1].lower() if '.' in doc.filename else ''
                mime_types = {
                    'pdf': 'application/pdf',
                    'jpg': 'image/jpeg',
                    'jpeg': 'image/jpeg',
        ...
        for doc in self:
            if doc.filename:
                ...
                if extension not in allowed_extensions:
                    raise ValidationError(_("File type not allowed. Allowed types: %s") % ', '.join(allowed_extensions))
    ...
    def create(self, vals):
        """Create document with automatic filename if not provided"""
        if not vals.get('filename'):
            document_type = dict(self._fields['document_type'].selection).get(vals.get('document_type'))
            vals['filename'] = _('%s - %s') % (document_type, fields.Date.today())
        
        return super(BeritLoanDocument, self).create(vals)
    ...
    def _send_expiry_notification(self):
        """Send notification about expired document"""
        template = self.env.ref('berit_loan.email_template_document_expired', raise_if_not_found=False)
        if template:
